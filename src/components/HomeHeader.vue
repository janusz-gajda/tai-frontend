    ...
    <div :class="twMerge(`h-fit bg-gradient-to-b from-emerald-800 p-6`)">
        <div class="w-full mb-4 flex items-center justify-between">
            ...
            <div v-if="userStore.id === 0" class="flex justify-between items-center gap-x-4">
                <div>
                    <LoginButton @click="modalStore.openModal('register')" class="bg-transparent text-neutral-300 font-medium">
                        Sign up
                    </LoginButton>
                </div>
                <div>
                    <LoginButton @click="modalStore.openModal('login')" class="bg-white px-6 py-2"> Log in </LoginButton>
                </div>
            </div>
            <div v-else class="flex justify-between items-center gap-x-4">
                <p class="text-white font-bold text-xl">Welcome, {{ userStore.name }}</p>
            </div>
        </div>
        <slot />
    </div>
