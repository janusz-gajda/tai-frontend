...
<template>
  <div class="refative w-full items-center flex">
    <h1>{{ convertSeconds(props.value) }}</h1>
    <SliderRoot
      class="relative mx-3 flex items-center justify-between select-none touch-none w-full h-10"
      v-model="value"
    ...
    <h1>{{ convertSeconds(props.maxValue) }}</h1>
  </div>
</template>
