...
<template>
    ...
    >
        <div class="flex flex-col w-full h-full" @click="handleClick">
            <p class="font-semibold truncate text-center py-3 mb-2 w-full">
                {{ props.playlist.name }}
            </p>
            <div class="relative aspect-square w-full h-full rounded-md overflow-hidden">
                <img :src="defaultCoverUrl" alt="Image" class="w-full object-cover rounded-md" />
            </div>
        ...
        <div @click="$emit('playPlaylist')" class="absolute bottom-16 right-5">
            ...
            >
                <BsPauseFill
                    size="20"
                    v-if="playerStore.isPlaying && playerStore.playlistId === props.playlist.id"
                    class="text-black"
                />
