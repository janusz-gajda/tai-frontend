<script setup lang="ts">
    ...
    import DropdownPlaylist from './DropdownPlaylist.vue';
...
import { addToPlaylist, removeFromPlaylist } from '@/api/collections';
    ...
    contentStore.getDataForDropdown(props.song.id).forEach((value, key) => {
    ...
    <DropdownMenuRoot v-model:open="menu">
        ...
        <DropdownMenuPortal>
            ...
            >
                ...
                <DropdownMenuItem v-for="(playlist, index) in contentStore.playlists" :key="playlist.id" value="Delete">
                        <DropdownMenuCheckboxItem class="relative flex pl-7 py-1 flex-row justify-start items-center hover:bg-neutral-800 rounded-md" v-model:checked="checkedRef[index]" @click="(event: any) => event.stopPropagation()" @select="(event) =>handleChange(event, playlist.id)">
