...
<template>
    ...
    <div
        v-else
        class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-8 gap-4 mt-4 mb-32"
    >
        <AlbumItem
        ...
        />
        <PlaylistItem
            @play-playlist="handlePlayPlaylist(playlist)"
            :playlist="playlist"
            v-for="playlist in props.playlists"
            :key="playlist.id"
        />
        <SongItem
            @play-song="playerStore.playSong(song)"
